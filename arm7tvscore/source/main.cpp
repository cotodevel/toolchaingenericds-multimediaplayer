...
u8 * NDSHeaderStruct = (u8*)( ((int)TGDSDLDI_ARM7_ADDRESS_TVS_CORE) + (16*1024)); //[4096]
#define NDSHeaderStruct_Size ((int)4096)
...
void bootfile(){
	...
	if (fresult != FR_OK) {
		...
		pf_lseek(0, currentFH);
		int isNTRTWLBinary = isNTROrTWLBinaryTGDSMB7(currentFH, NDSHeaderStruct, NDSHeaderStruct_Size, ARM7i_HEADER_SCFG_EXT7, &isTGDSTWLHomebrew);
		...
		if(
			...
			UINT nbytes_read;
			pf_lseek(0, currentFH);
			memset(NDSHeaderStruct, 0, NDSHeaderStruct_Size);
			pf_read((u8*)NDSHeaderStruct, NDSHeaderStruct_Size, &nbytes_read, currentFH);
...
//---------------------------------------------------------------------------------
	...
	ARM7InitDLDI(TGDS_ARM7_MALLOCSTART, TGDS_ARM7_MALLOCSIZE, TGDSDLDI_ARM7_ADDRESS_TVS_CORE);
