...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  if((ret=_fetch_headers(vf,&vf->vi,&vf->vc,&vf->current_serialno,NULL))<0){
	...
	bool closeOGGFileHandleInternally = true;
...
static int _ov_open2(OggVorbis_File *vf){
  ...
  if(vf->seekable){
    ...
    if(ret){
      vf->datasource=NULL;
	  bool closeOGGFileHandleInternally = true;	//no, can't close the filehandle here through callbacks. Close it through the DRAGON_xxxx library
	  ov_clear(vf, closeOGGFileHandleInternally);
    }
    return ret;
  }
...
int ov_clear(OggVorbis_File *vf, bool closeFileHandle){
  if(vf){
    vorbis_dsp_destroy(vf->vd);
    vf->vd=0;
	...
	//coto: decide when the file handle is closed
	if(closeFileHandle == true){
		if(vf->datasource){
