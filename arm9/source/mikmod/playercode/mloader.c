...
static void ML_XFreeSample(SAMPLE *s)
{
	...
	if(s->samplename) safeFree(s->samplename);
}

static void ML_XFreeInstrument(INSTRUMENT *i)
{
	if(i->insname) safeFree(i->insname);
}
...
static void ML_FreeEx(MODULE *mf)
{
	...
	if(mf->songname) safeFree(mf->songname);
	if(mf->comment)  safeFree(mf->comment);

	if(mf->modtype)   safeFree(mf->modtype);
	if(mf->positions) safeFree(mf->positions);
	if(mf->patterns)  safeFree(mf->patterns);
	if(mf->pattrows)  safeFree(mf->pattrows);

	if(mf->tracks) {
		for(t=0;t<mf->numtrk;t++)
			if(mf->tracks[t]) safeFree(mf->tracks[t]);
		safeFree(mf->tracks);
	}
	if(mf->instruments) {
		for(t=0;t<mf->numins;t++)
			ML_XFreeInstrument(&mf->instruments[t]);
		safeFree(mf->instruments);
	}
	if(mf->samples) {
...
MODULE* Player_LoadGeneric_internal(MREADER *reader,int maxchan,BOOL curious)
{
	...
	/* safeFree loader and unitrk allocations */
