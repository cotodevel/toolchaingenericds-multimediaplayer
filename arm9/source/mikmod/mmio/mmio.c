...
VIRTUAL_FILE* _mm_fopen(CHAR* fname,CHAR* attrib)
{
	...
	else
	{
		...
		fread(vfp->buffer, 1, vfp->size, fp);
		fclose(fp);
	}
...
static BOOL _mm_FileReader_Read(MREADER* reader,void* ptr,size_t size)
{
	...
	//return fread(ptr, 1, size, ((MFILEREADER*)reader)->file);
}
...
static int _mm_FileReader_Get(MREADER* reader)
{
	...
	//fread(&c, 1, 1, ((MFILEREADER*)reader)->file);
...
static BOOL _mm_FileReader_Seek(MREADER* reader,long offset,int whence)
{
	...
	//return fseek(((MFILEREADER*)reader)->file, (whence==SEEK_SET)? offset+_mm_iobase:offset, whence);
}
...
{
	return false;
	//return (fwrite(ptr, 1, size, ((MFILEWRITER*)writer)->file)==size);
}
