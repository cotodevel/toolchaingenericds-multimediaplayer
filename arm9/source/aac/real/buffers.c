...
AACDecInfo *AllocateBuffers(void)
{
	...
	aacDecInfo = (AACDecInfo *)safeMalloc(sizeof(AACDecInfo));
	if (!aacDecInfo)
		return 0;
	ClearBuffer(aacDecInfo, sizeof(AACDecInfo));

	aacDecInfo->psInfoBase = safeMalloc(sizeof(PSInfoBase));
...
#ifndef SAFE_FREE
#define SAFE_FREE(x)	{if (x)	safeFree(x);	(x) = 0;}	/* helper macro */
#endif
